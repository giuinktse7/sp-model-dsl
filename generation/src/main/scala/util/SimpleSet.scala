...
object SimpleSet {
  ...
  /**
    * @return An Optional lens into a util.SimpleSet for the value associated with the key id.
    */
...
/**
  ...
  *
  *   // New
  *   val models = new util.SimpleSet(_.id, Map[Int, Model])
  *
  *   // Update
  *   models + newModel
  *
...
class SimpleSet[K, V](hashBy: V => K, private val data: Map[K, V]) {
  ...
  override def toString: String = s"util.SimpleSet(${data.values.mkString(", ")})"
}
