...
import codegen.model.{Action, EffectConditional}

sealed trait LogMessage {
  def msg: String
}

case class ValuesNotInDomain(condition: EffectConditional[Unit], actions: NonEmptyList[Action]) extends LogMessage {
...
case class ConditionGroupMissing(condition: EffectConditional[Unit])(implicit ctx: EvalContext) extends LogMessage {
